...
import { resolveWorker, resolveWorkerFromTrace } from '$lib/config/workers';
...
function deriveFriendlyWorker(event: RawLogEvent): string {
	// Worker identity is registry-driven — resolve from the explicit worker
	// field first, then fall back to the trace_id prefix.
	const def = resolveWorker(event.worker) ?? resolveWorkerFromTrace(event.trace_id);
	return def?.label ?? event.worker ?? 'System';
}
