<script lang="ts">
	...
	import { Brain, Clock, Globe, ShieldCheck, Microscope, Info } from 'lucide-svelte';
		...
		<div class="space-y-12">
			...
			{#if data.lessons.length > 0}
				<section>
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						{#each data.lessons as lesson (`${lesson.adopted_date}:${lesson.title ?? lesson.text.slice(0, 40)}`)}
							<div class="bg-blue-500/5 border border-blue-500/20 rounded-lg p-5 transition-all hover:bg-blue-500/10 hover:border-blue-500/40">
								...
								<div class="flex flex-wrap items-center gap-3 pt-3 border-t border-blue-500/10">
									<div class="flex items-center gap-1.5 text-[10px] text-slate-500 uppercase">
										<Clock size={12} />
										Added {formatDate(lesson.adopted_date)}
									</div>
									<div class="flex items-center gap-1.5 text-[10px] text-slate-500 uppercase">
										<Globe size={12} />
			...
			<section>
				...
				<div class="space-y-2">
					{#if data.raw.length > 0}
						{#each data.raw as obs (obs.observation_id)}
							<div class="bg-slate-900/40 border border-slate-800 rounded p-3 flex gap-4 items-start transition-all hover:bg-slate-900/80 group">
								...
								<div class="flex-1 min-w-0">
									<div class="flex items-center gap-3 mb-1">
										...
										{#if obs.ticket_id}
											<span class="text-[10px] text-slate-500 font-mono">{obs.ticket_id}</span>
										{/if}
									...
									<p class="text-xs text-slate-400 leading-relaxed">
										{obs.text.length > 140 ? obs.text.slice(0, 140) + '…' : obs.text}
									</p>
						...
						{/each}
					{:else}
						<div class="text-center py-6 text-[11px] text-slate-600 italic border border-slate-800 rounded">
			...
			{#if data.provisional.length > 0}
				<section>
					...
					<p class="text-[10px] text-slate-600 italic mb-4">These patterns are provisional — not yet confirmed across multiple jobs</p>

					<div class="space-y-3">
						{#each data.provisional as lesson (lesson.id)}
